...
- **The complete test suite must finish in under one second.** `uv run pytest` must run the entire repository test suite and complete in less than 1.0 second of wall-clock time once the development environment is installed. There is no slow-test allowance. Delete or rewrite tests that require real sleeps, long polling, repeated nested test-suite runs, heavyweight service/process startup, stress loops, or other intrinsically slow behavior. Prefer deterministic fakes, injected clocks/state, and direct invariant checks.
