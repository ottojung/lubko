...
def test_abort_gated_start_blocks_on_unproven_group(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """abort_gated_start reports blocking failure when the group survives.

    Fault injection: the exact gated group is made to look permanently alive.
    The abort must NOT report success, must retain the capture files for
    diagnosis, and callers must then skip terminalization so the running row
    keeps its persisted exact identity recoverable.
    """
    sentinel = tmp_path / "ran"
    proc, stdout_path, stderr_path, pgid, gate_fd = worker_mod.spawn_job(
        worker_mod.Job(
    ...
    guard.register(proc)
    real_has_members = worker_mod.group_has_members
    ...
    monkeypatch.setattr(worker_mod, "group_has_members", _survives)
    try:
        aborted = worker_mod.abort_gated_start(proc, pgid, stdout_path, stderr_path, gate_fd)
        assert aborted is False
        # The user program must never have run and the proof failure retains
        # the capture files for diagnosis instead of claiming a clean abort.
        assert not sentinel.exists()
