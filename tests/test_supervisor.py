...
#: A two-column table without the canonical v3 output-chunk shape (no
...
def insert_job(conninfo: str, cwd: str, command: str) -> UUID:
    """Insert a protocol v3 pending shell-builtin command job.

    The worker never invokes a shell itself, so a command string is wrapped in
    an explicit ``["bash", "-lc", command]`` argv chosen by the test.

    Args:
        conninfo: PostgreSQL connection string.
        cwd: Working directory for the job.
        command: Shell command wrapped explicitly in a ``bash -lc`` argv.

    Returns:
        The job identifier.
    """
    return insert_process_job(conninfo, cwd, ["bash", "-lc", command])


def insert_process_job(conninfo: str, cwd: str, process: list[str]) -> UUID:
    """Insert a protocol v3 pending process-argv job.

    Args:
        conninfo: PostgreSQL connection string.
        cwd: Working directory for the job.
        process: argv-style command to exec directly, with no shell.
...
def test_v3_worker_refuses_pre_canonical_shape(jobs_db: str, pg_cluster: _pg.PgCluster) -> None:
...
def test_process_argv_keeps_shell_metacharacters_literal(
    jobs_db: str, pg_cluster: _pg.PgCluster, tmp_path: Path
) -> None:
    """A process argv with shell metacharacters is executed literally, with no shell."""
    literal = "$HOME; echo hacked >&2; * ? | && `id`"
    job_id = insert_process_job(jobs_db, str(tmp_path), ["printf", "%s\n", literal])
    ...
    assert payload["state"]["status"] == "succeeded"
    assert payload["result"]["exit_code"] == 0
    assert payload["result"]["stdout"] == literal + "\n"
    assert not payload["result"]["stderr"]


def test_v2_payload_is_rejected_and_later_work_succeeds(
    jobs_db: str, pg_cluster: _pg.PgCluster, tmp_path: Path
) -> None:
    """A surviving v2 row fails with a clear diagnostic; v3 work still succeeds."""
    v2_payload = json.dumps({
        "v": 2,
        "type": "command",
        "request": {"cwd": str(tmp_path), "command": "echo old"},
        "state": {"status": "pending"},
    })
    with psycopg.connect(jobs_db) as conn:
        row = conn.execute(
    ...
    assert row is not None
    v2_id = cast("UUID", row[0])
    healthy = insert_process_job(jobs_db, str(tmp_path), ["echo", "new"])

    with supervisor_running(supervisor_settings(), make_database_config(pg_cluster), jobs_db):
        wait_until(lambda: read_status(jobs_db, healthy) == "succeeded")
        wait_until(lambda: read_status(jobs_db, v2_id) == "failed")

    payload = read_root(jobs_db, v2_id)
    assert payload["state"]["status"] == "failed"
    assert payload["result"]["exit_code"] == 2
    assert "v2 payloads are no longer accepted" in payload["result"]["stderr"]
...
def test_job_runs_in_declared_working_directory(
    ...
    """A shell-argv and a process-argv job run inside their declared cwd."""
    work_dir = tmp_path / "runner"
    work_dir.mkdir()
    shell_id = insert_job(jobs_db, str(work_dir), "pwd")
    argv_id = insert_process_job(
        jobs_db,
        str(work_dir),
...
def test_process_runs_in_declared_cwd(
    jobs_db: str, pg_cluster: _pg.PgCluster, tmp_path: Path
) -> None:
    """A process argv job executes inside its declared cwd, not the supervisor's."""
    work_dir = tmp_path / "runner"
    work_dir.mkdir()
...
def test_missing_process_fails_cleanly_and_later_work_succeeds(
    jobs_db: str, pg_cluster: _pg.PgCluster, tmp_path: Path
) -> None:
    """A claimable v3 row without a process argv fails cleanly without harming others."""
    ...
    with supervisor_running(supervisor_settings(), make_database_config(pg_cluster), jobs_db):
        wait_until(lambda: read_status(jobs_db, healthy) == "succeeded")
        wait_until(lambda: read_status(jobs_db, bad_id) == "failed")
        subsequent = insert_process_job(jobs_db, str(tmp_path), ["echo", "after"])
        wait_until(lambda: read_status(jobs_db, subsequent) == "succeeded")

    payload = read_root(jobs_db, bad_id)
    assert payload["state"]["status"] == "failed"
    assert payload["result"]["exit_code"] == 2
    assert "request.process is required" in payload["result"]["stderr"]
    assert read_status(jobs_db, healthy) == "succeeded"
