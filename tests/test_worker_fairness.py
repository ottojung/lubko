...
def test_gc_failure_still_advances_its_schedule(monkeypatch: pytest.MonkeyPatch) -> None:
    """A failed optional GC pass waits for the next cadence before retrying."""
    settings = Settings(
        worker_id="worker",
        server="server",
        poll_interval_seconds=0.1,
