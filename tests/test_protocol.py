...
def test_build_payload_process() -> None:
    """build_payload emits a claimable protocol v3 process payload."""
...
def test_build_payload_rejects_empty_process() -> None:
    """An empty process array is a binding violation."""
    with pytest.raises(ProtocolError, match=r"request\.process"):
        build_payload(cwd="/x", process=[])


def test_build_payload_rejects_empty_process_element() -> None:
    """An empty string inside process is a binding violation."""
    with pytest.raises(ProtocolError, match=r"request\.process"):
        build_payload(cwd="/x", process=["git", ""])


def test_build_payload_rejects_non_string_process_element() -> None:
    """A non-string element inside process is a binding violation."""
    with pytest.raises(ProtocolError, match=r"request\.process"):
        build_payload(cwd="/x", process=["git", 3])  # type: ignore[list-item]
...
def test_parse_payload_rejects_unsupported_version() -> None:
    """Unknown protocol versions are refused with a generic diagnostic."""
...
def test_parse_payload_rejects_v2_payload() -> None:
    """A protocol v2 payload is explicitly rejected with a clear diagnostic."""
    with pytest.raises(ProtocolError, match="v2 payloads are no longer accepted"):
        parse_payload({
            "v": 2,
            "type": JOB_TYPE_COMMAND,
...
def test_parse_payload_rejects_missing_process() -> None:
    """A v3 request without a process argv is a binding violation."""
    with pytest.raises(ProtocolError, match=r"request\.process is required"):
        parse_payload({
            "v": 3,
            "type": JOB_TYPE_COMMAND,
            "request": {"cwd": "/x"},
...
def test_parse_payload_rejects_malformed_process() -> None:
    """An empty or non-string process argv is a binding violation."""
    for process in [[], [""], ["ls", ""], ["ls", None], ["ls", 7]]:
        with pytest.raises(ProtocolError, match=r"request\.process"):
            parse_payload({
                "v": 3,
                "type": JOB_TYPE_COMMAND,
                "request": {"cwd": "/x", "process": process},
                "state": {"status": STATUS_PENDING},
            })


def test_parse_payload_rejects_legacy_command_with_process() -> None:
    """request.command alongside a valid process is rejected, not tolerated."""
    with pytest.raises(ProtocolError, match="only process"):
        parse_payload({
            "v": 3,
            "type": JOB_TYPE_COMMAND,
...
def test_parse_payload_rejects_legacy_args_with_process() -> None:
    """request.args alongside a valid process is rejected, not tolerated."""
    with pytest.raises(ProtocolError, match="only process"):
        parse_payload({
            "v": 3,
            "type": JOB_TYPE_COMMAND,
...
def test_parse_payload_rejects_legacy_command_alone() -> None:
    """A bare v3 request.command is rejected as a legacy field."""
    with pytest.raises(ProtocolError, match="only process"):
        parse_payload({
            "v": 3,
            "type": JOB_TYPE_COMMAND,
...
def test_parse_chunk_payload_rejects_unsupported_version() -> None:
    """Chunks at an unsupported protocol version are rejected."""
    with pytest.raises(ProtocolError, match="no longer accepted"):
        parse_chunk_payload({
            "v": 2,
