...
def make_active_job(tmp_path: Path, *, process: tuple[str, ...] = ("echo", "hi")) -> ActiveJob:
...
def test_group_has_members_tracks_process_group(tmp_path: Path) -> None:
    ...
    proc, _stdout_path, _stderr_path, pgid = spawn_job(
        Job(id=uuid4(), cwd=str(tmp_path), process=("sleep", "30"))
    )
    guard.register(proc)
    try:
...
def test_spawn_job_runs_process_and_cleanup_files(tmp_path: Path) -> None:
    ...
    proc, stdout_path, stderr_path, _pgid = spawn_job(
        Job(id=uuid4(), cwd=str(tmp_path), process=("echo", "hi"))
    )
    guard.register(proc)
    try:
        proc.wait(timeout=10)
...
def test_spawn_job_keeps_shell_metacharacters_literal(tmp_path: Path) -> None:
    """Shell metacharacters in argv stay literal and are never evaluated."""
    literal = "$HOME; echo hacked >&2; * ? | && `id`"
    proc, stdout_path, stderr_path, _pgid = spawn_job(
        Job(id=uuid4(), cwd=str(tmp_path), process=("printf", "%s\n", literal))
    )
    guard.register(proc)
    try:
        proc.wait(timeout=10)
...
def test_request_group_reap_preserves_natural_status(tmp_path: Path) -> None:
    ...
    proc, _stdout_path, _stderr_path, pgid = spawn_job(
        Job(
            id=uuid4(),
            cwd=str(tmp_path),
            process=("bash", "-lc", "sleep 30 & echo done"),
        )
    )
    guard.register(proc)
    try:
