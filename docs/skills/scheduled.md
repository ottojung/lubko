...
- **Lubko** — the execution/orchestration platform: the server that runs queued development jobs (executed directly as argv; shell use only when a submitted argv explicitly selects a shell) and managed `lubko-agent` sessions, plus its transport (`lubko.jobs`), agent state, worktrees/checkouts, and execution state.
